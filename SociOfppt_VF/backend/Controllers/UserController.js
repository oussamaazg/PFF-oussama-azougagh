...
export const searchUser= async (req,res) =>{
  ...
  try{
    ...
    if (user) {
      ...
      res.status(404).json("no user exist with this lastname")
    }
  }catch (err){
...
export const followUser = async (req, res) => {
  ...
  if (_id === id) {
    res.status(403).json("you have not the access to follow your self");
  } else {
    try {
...
export const UnFollowUser = async (req, res) => {
  ...
  if (_id === id) 
  {
    res.status(403).json("you have not the access to unfollow your self");
  } 
  else {
    try {
...
export const FollowersUser= async(req , res)=>{
  // try {
        const user = await UserModel.findById(req.params.id);
